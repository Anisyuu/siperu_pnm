            ...
            <table class="w-full text-sm">
                ...
                @forelse($peminjaman as $p)
                    @php
                        ...
                        $semuaLangkah = $p->verifikasi->map(fn($v) => [
                            'urutan'                  => $v->urutan,
                        ...
                        <td class="px-5 py-4 text-center">
                            @if ($p->status == 'pending' && $p->isBisaDiAksi($firstPendingId))
                                <button type="button" onclick="openModal(this)"
                                    ...
                                    data-semua-langkah='@json(json_decode($semuaLangkah, true))'
...
<div id="detailModal"
    ...
    <div id="modalPanel"
        ...
        {{-- Modal Footer Aksi --}}
        <div id="modalFooter"
            class="px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between gap-3 rounded-b-2xl">

            {{-- Form Tolak --}}
            <form id="formTolak" action="" method="POST" class="flex items-center gap-2 flex-1">
                @csrf
                @method('PATCH')
            ...
            {{-- Form Setujui --}}
            <form id="formSetujui" action="" method="POST">
                @csrf
                @method('PATCH')
        ...
        <div id="modalFooterDone"
            class="hidden px-6 py-4 border-t border-slate-100 bg-slate-50/50 flex justify-end rounded-b-2xl">
            <button type="button" onclick="closeModal()"
...
<script>
    ...
    function openModal(btn) {
        ...
        if (d.status === 'pending') {
            formSetujui.action = d.approveUrl || '';
            formTolak.action = d.rejectUrl || '';
    ...
    document.getElementById('formSetujui').addEventListener('submit', function(e) {
        if (!this.action) {
            e.preventDefault();
            alert('URL approve tidak ditemukan.');
        }
    });
    ...
    document.getElementById('formTolak').addEventListener('submit', function(e) {
        if (!this.action) {
            e.preventDefault();
            alert('URL reject tidak ditemukan.');
        }
    });
</script>
@endpush
