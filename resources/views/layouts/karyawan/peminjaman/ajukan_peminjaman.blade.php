...
<div class="max-w-5xl mx-auto">
    ...
    <form action="{{ route('karyawan.simpan-peminjaman') }}" method="POST" enctype="multipart/form-data" id="formPeminjaman">
        @csrf
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadRuanganTersedia() {
        ...
        try {
            const response = await fetch(`{{ route('karyawan.ruangan-tersedia') }}?${params.toString()}`, {
                headers: {
