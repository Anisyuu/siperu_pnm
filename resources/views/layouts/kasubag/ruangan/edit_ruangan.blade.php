...
<div class="max-w-3xl mx-auto p-10 bg-white rounded-2xl border border-gray-200 shadow-sm">
    ...
    <!-- FORM -->
    <form action="{{ route('kasubag.update-ruangan', $ruangan->id) }}" method="POST" class="flex flex-col gap-6">
        @csrf
        @method('PUT')
            ...
            <select name="id_jenis_ruang"
                ...
                @foreach($jenisRuang as $jenis)
                    <option value="{{ $jenis->id }}"
                        {{ old('id_jenis_ruang', $ruangan->id_jenis_ruang) == $jenis->id ? 'selected' : '' }}>
                        {{ $jenis->nama }}
                    </option>
                @endforeach
        ...
        <div>
            ...
            <input
                ...
                name="nomor_ruang"
                maxlength="5"
                value="{{ old('nomor_ruang', $ruangan->nomor_ruang) }}"
        ...
        <div>
            ...
            <input
                ...
                name="nama_ruang"
                maxlength="25"
                value="{{ old('nama_ruang', $ruangan->nama_ruang) }}"
