...
<div class="max-w-7xl mx-auto flex flex-col gap-6">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <a href="{{ route('kasubag.riwayat-peminjaman.export', request()->query()) }}"
                ...
                <tr class="hover:bg-slate-50/70 transition-colors group">
                    ...
                    <td class="px-5 py-4 text-center">
                        <button onclick="openModal(this)"
                            ...
                            data-catatan-peminjaman="{{ $p->catatan ?? '' }}"
                ...
                <tr>
                    <td colspan="7" class="px-6 py-16 text-center">
                        <div class="flex flex-col items-center gap-3">
                            ...
                            <div>
                                <p class="font-semibold text-slate-500 text-sm">Belum ada riwayat peminjaman</p>
                                <p class="text-xs text-slate-400 mt-0.5">Riwayat akan muncul setelah terdapat pengajuan peminjaman.</p>
                            </div>
                        </div>
                    </td>
                </tr>
...
<div id="detailModal"
    ...
    <div id="modalPanel"
        ...
        <div class="flex items-start justify-between px-6 py-5 border-b border-slate-100">
            <div>
                <h3 class="text-lg font-extrabold text-slate-900">Detail Riwayat Peminjaman</h3>
                <p class="text-xs text-slate-400 mt-0.5">Informasi lengkap riwayat pengajuan peminjaman ruangan</p>
            </div>
        ...
        <div class="overflow-y-auto max-h-[70vh]">
            ...
            <div class="px-6 py-5 space-y-6">
                ...
                <div class="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-xl px-4 py-3">
                    ...
                    <span id="modal_status_akhir" class="px-3 py-1 text-xs font-bold rounded-full"></span>
                </div>
                ...
                {{-- CATATAN PEMINJAMAN --}}
                <div id="catatanPeminjamanWrap" class="hidden">
                    <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-3 flex items-center gap-1.5">
                        <i class="fa-solid fa-comment-dots text-amber-400"></i> Catatan Peminjaman
                    </p>

                    <div class="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 text-sm text-amber-800">
                        <p id="modal_catatan_peminjaman"></p>
                    </div>
                </div>
...
<script>
    ...
    function openModal(btn) {
        ...
        // Status Akhir Peminjaman
        const badgeHeader = document.getElementById('modal_status_badge');
        const badgeAkhir  = document.getElementById('modal_status_akhir');

        setBadge(badgeHeader, d.status);
        ...
        // Catatan Peminjaman
        const catatanPeminjamanWrap = document.getElementById('catatanPeminjamanWrap');
        const catatanPeminjamanEl = document.getElementById('modal_catatan_peminjaman');

        if (d.catatanPeminjaman && d.catatanPeminjaman.trim() !== '') {
            catatanPeminjamanEl.textContent = d.catatanPeminjaman;
            catatanPeminjamanWrap.classList.remove('hidden');
        } else {
            catatanPeminjamanWrap.classList.add('hidden');
        }
