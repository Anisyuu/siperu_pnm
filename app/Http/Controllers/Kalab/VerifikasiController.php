...
class VerifikasiController extends Controller
{
    private function cekGiliran(Peminjaman $peminjaman): array
    {
        ...
        $isLab = str_contains(
            strtolower(trim(optional($peminjaman->ruangan->jenisRuangan)->nama ?? '')),
            'lab'
        );
        ...
        Log::debug('[cekGiliran]', [
         ...
         // 3. Ambil alur verifikasi
        $alurSemua = AlurVerifikasi::whereRaw('LOWER(TRIM(jenis_pemohon)) = ?', [$jenisPemohon])
            ->orderBy('urutan')
            ->get();

        if ($alurSemua->isEmpty()) {
            return [
                'boleh' => false,
        ...
        // 4. Filter alur — skip step kalab jika ruangan bukan lab
        $alur = $alurSemua->filter(function ($step) use ($isLab) {
            $roleStep = strtolower(trim($step->role_verifikator));
            if (!$isLab && $roleStep === 'kalab') {
                return false; // lewati step kalab untuk ruangan non-lab
            }
            return true;
        })->values(); // re-index agar iterasi aman

        // 5. Ambil semua record verifikasi yang sudah ada
        $tercatat = Verifikasi::where('id_peminjaman', $peminjaman->id)
            ->get()
            ->keyBy('urutan');

        // 6. Jika ada yang ditolak → stop
