...
class PeminjamanController extends Controller
{
    ...
    private function kirimNotifikasiKeRole(string $role, Peminjaman $peminjaman): void
    {
        ...
        | Khusus Sarpras
        |--------------------------------------------------------------------------
        | Sarpras diambil dari gedung yang dipilih.
        |
        | Struktur relasi sistem:
        | ruangan.id_gedung = gedung.id
        ...
        */
        if ($role === 'sarpras') {
            ...
            if (!$sarprasGedung) {
                Log::warning('[notifikasi peminjaman] user sarpras pada gedung tidak ditemukan', [
                    'peminjaman_id' => $peminjaman->id,
                    'ruangan_id'    => $peminjaman->ruangan_id,
                    'gedung_id'     => $gedung->id,
            ...
            if (!$sarprasGedung->roles->contains(fn ($role) => strtolower(trim($role->nama)) === 'sarpras')) {
                Log::warning('[notifikasi peminjaman] user pada gedung bukan role sarpras', [
                    'peminjaman_id'       => $peminjaman->id,
                    'gedung_id'           => $gedung->id,
                    'gedung_nama'         => $gedung->nama ?? null,
            ...
            Log::info('[notifikasi peminjaman] dikirim ke sarpras gedung', [
                ...
                'sarpras_nama'        => $sarprasGedung->nama_lengkap,
            ]);
        ...
        | Role Selain Sarpras
        |--------------------------------------------------------------------------
        | Kasubag, kalab, pimpinan, dan role lain tetap dikirim berdasarkan role.
        */
    ...
    public function listPeminjaman(Request $request)
    {
        ...
        return view('layouts.dosen.peminjaman.list_peminjaman', compact('peminjaman'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            return redirect()->route('dosen.list-peminjaman')
                ->with('success', 'Pengajuan berhasil dikirim.');

        } catch (\Exception $e) {
            Log::error('[store peminjaman dosen] error', [
    ...
    public function detailPeminjaman($id)
    {
        $peminjaman = Peminjaman::with([
            ...
            return view('layouts.dosen.peminjaman.detail_peminjaman', compact(
                'peminjaman',
                'alur',
                'riwayat'
            ));
    ...
    public function exportRiwayatPeminjaman(Request $request)
    {
        ...
        $fileName = 'riwayat-peminjaman-dosen-' . now()->format('Ymd_His') . '.csv';
